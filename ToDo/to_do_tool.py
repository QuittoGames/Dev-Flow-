...
class to_do_tool:
    def Add_Task():
        ...
        time_notificate = input("Digite O Tempo De Espera Da Notificaçao (S): ")
        time = int(time_notificate)
    ...
    def task_timer(task):
        ...
        tool.Notification(task)
        task.state = True
    
    def Show_Menu_Task(task):
        for tarefa in task:
    ...
    def Show_Task_Pendentes():
        ...
        if len(data.Tasks_to_do) == 0:
            ...
            for task in data.Tasks_to_do:
                print(f"Nome: {task.name_task} | Tempo: {task.timer}s | Status: {task.status}")
